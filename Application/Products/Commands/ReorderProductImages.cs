...
public sealed class ReorderProductImages
{
        ...
        : IRequestHandler<Command, Result<Unit>>
    {
        public async Task<Result<Unit>> Handle(Command request, CancellationToken ct)
        {
            ...
            // Load all non-deleted images belonging to this product (both product-level and variant-level)
            List<ProductImage> images = await context.ProductImages
                .Where(i =>
                    !i.IsDeleted &&
                    (i.ProductId == request.ProductId ||
                     (i.ProductVariantId != null && i.ProductVariant!.ProductId == request.ProductId)))
                .ToListAsync(ct);
